...
Рекомендации намеренно денормализованы так, чтобы для каждого пользователя хранить рекомендуемые ему посты. Также будет доп таблица,
которая хранит idUser и последние 10 выложенных постов, в будущем это будет использоваться
...
нагрузку не только на запись в БД, но и на чтение для комментов. Поэтому мы будем использовать реплики,
...
больше памяти чем в действительности нужно, также эту отдельную БД удобно использовать в микросервисной архитектуре в сочетании
с микросервисом комментариев.
Также применим шардирование у комментариев по id поста, для ускорения записи.
...
Также часть нагрузки на составление ленты, будет распределятья на PostgreSql + Redis в случае получения человеком постов по
...
PostgreSql будет использоваться для поиска id всех друзей, на которых подписан пользователь, а затем мы запросим у Redis
по id последние 10 постов каждого друга, отсортируем их по времени и по лайкам.
...
Также для увеличения скорости чтения важно помимо реплик использовать правильные индексы. Так как все `primary key`
уже имеют на себе индексы по дефолту, их расписывать не будем.
Возьмем следующие индексы:

* Индекс на id подписчика (id1 того кто подписывается) в таблице подписок
* Индекс на id поста в комментариях, для того чтобы быстрее собирать комментарии к заданному посту
* Индекс на телефон у пользователя, для того чтобы быстрее находить юзера по номеру телефона
* В БД Redis "Ленты", добавить индексы на дату создания и число лайков.
* Индекс на id пользователя, поставившего лайк, в таблице лайков.


![Физическая схема БД](https://github.com/Amaterasy17/HighLoad-courseWork/raw/main/images/highload_shard.svg)
...
1. *Выбор оборудования под сервера и его конфигурация*
    ...
    * сервера СУБД Redis рекомендаций
    * сервера СУБД Redis ленты
    * балансировщика

*Балансировщик*

Из личного практического опыта, Nginx с протоколом HTTP выдает на компьютере с 4 логическими ядрами выдает *~12000 rps*
Но мы будем использовать SSL, тогда Nginx на одном ядре будет выдавать
~ 250 rps. 
Возьмем тогда массовый современный процессор Intel E5-2660 v4 – 14 ядер. Он будет
выдавать *14 * 2 * 2 * 250 rps = 14 000 rps*
Четырнадцать балансировщиков с такой конфигурацией нам будет хватать в притык исходя из нашей нагрузки. Для будущего масштабирования,
DNS балансировки возьмем 28 балансировщика нагрузки с данной конфигурацией.
...
располагаются на той же машине что и Nginx, так как в основном они отдают сбандленную статику, фронтовых серверов будет как и 
балансировщиков 28.
...
Можно заметить линейный рост при чтении при увеличении числа ядер. Значит, 1 бэк с 56 ядрами, 256Гб ОЗУ даст примерно
...
То есть у данного сервера будет 256Гб ОЗУ + массовый процессор Intel E5-2620 v4 – 8 ядер
...
* Пользователь ~ 1300 байт/запись * [460млн](https://ru.wikipedia.org/wiki/%D0%92%D0%9A%D0%BE%D0%BD%D1%82%D0%B0%D0%BA%D1%82%D0%B5#:~:text=%D0%9F%D0%BE%20%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D0%BC%20%D0%BD%D0%B0%20%D0%B0%D0%B2%D0%B3%D1%83%D1%81%D1%82%202017,%D0%BC%D0%B5%D1%81%D1%82%D0%BE%20%D0%BF%D0%BE%20%D0%BF%D0%BE%D0%BF%D1%83%D0%BB%D1%8F%D1%80%D0%BD%D0%BE%D1%81%D1%82%D0%B8%20%D0%B2%20%D0%BC%D0%B8%D1%80%D0%B5.) = 584 Гб
* Пост ~ 2500 байт/запись * 310 млн * 12 ~ 10 Тб данных/год
* Подписки ~ 12 байт/запись, учитывая, что каждый подписан в среднем на 1000 человек, тогда выделим примерно 20Тб
...
* Темы ~ 200 байт/запись, примерно 2Тб
  ...
  Итого нужно с запасом: 120 Тб SATA памяти для лайков и подписок, а
...
Здесь также важно рассчитать нужное число ОЗУ для данной СУБД, так как
один пост ~ 2500 байт запись, для каждого пользователя мы будем хранить по 10 постов, так как активных пользователей
97 млн в месяц, пусть Redis хранит инфомацию о 190 млн пользователях.
Тогда получается `190 млн * 2500 байт/запись * 10` ~ 4,5 Тб ОЗУ. Так как максимальная конфигурация 1 сервера 1.5 ТБ ОЗУ,
будем использовать 3 сервера, между которыми по id пользователя будем шардить записи. Также будем использовать реплики,
для данной совокупности серверов, пусть будет 2 реплики для чтения.
Итого нам нужно 9 серверов с данной конфигурацией.

*Итого*

| Оборудование      | CPU + ядра                  | RAM, Гб | SSD, Tб |   HDD, Tб  |  количество     |
| ----------------- | ------------------| ------- | ------- | ----------- | -------------------------------|         
| Балансировщик + Front|Intel E5-2620 v4 – 8 ядер |   256    |    2 * 1 |     -    |     28  |
| backend            |Intel E5-2660 v4 – 14 ядер  |   256    |    1    |      1      |      35  |
| service(C++)  |    Intel E5-2620 v4 – 8 ядер    |   256    |    1    |      -      |       10 |
| PostgreSQL    |   Intel E5-2660 v4 – 14 ядер     |   512    |    215    |      120     |     4   |
| PostgreSQL комментов    |   Intel E5-2660 v4 – 14 ядер     |   512    |    300    |      -     |     15   |
| Redis рекомендаций      |    Intel E5-2620 v4 – 8 ядер     |   1536    |    1    |      -      |     2   |
| Redis  ленты      |    Intel E5-2620 v4 – 8 ядер     |   1536    |    1    |      -      |    9  |
...
4. *Отказоустойчивость*
   ...
   * Наличие L7 балансировки, причем, мы настроим балансировку следующим образом:
        1. 5 бекендов будут в состоянии "запасных", мы будем включать их в балансировку,
   ...
   * Также будем использовать реплики Redis для ленты, по схеме _Master-Slave(2)_,
     чтобы выдерживать нагрузку на сборку ленты.  В `Master` будем добавлять новые посты, а читать будем из реплик.
